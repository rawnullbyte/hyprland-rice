...
# 4. EXTRACT FRAME FOR PYWAL (if video) OR USE DIRECTLY (if image)
if [ "$is_video" = true ]; then
    # Create cache directory if it doesn't exist
    mkdir -p "$VIDEO_CACHE_DIR"
    
    # Extract first frame from video for pywal
...
# 5. GENERATE COLORS (Pywal) - using extracted frame or direct image
wal -i "$color_source" -n -q

# 6. GENERATE ROFI THEME WITH PYWAL COLORS
if [ -f "$HOME/.cache/wal/colors-rofi-dark.rasi" ]; then
    cat "$HOME/.cache/wal/colors-rofi-dark.rasi" > "$HOME/.config/rofi/colors.rasi"
fi

# 7. UPDATE HYPLOCK COLORS WITH PYWAL

...
Item {
    ...
    // Close all popups except the one passed
    function closeOtherPopups(except) {
        if (bluetoothPopup && bluetoothPopup !== except) bluetoothPopup.shouldShow = false
        if (networkPopup && networkPopup !== except) networkPopup.shouldShow = false
    ...
    Item {
        ...
        Rectangle {
         ...
         // ═══════════════════════════════════════════════════════════════
        // WEATHER MODULE - Between media and clock
        // ═══════════════════════════════════════════════════════════════
        Rectangle {
            id: weatherModule
            ...
            anchors.verticalCenter: parent.verticalCenter
            height: 28
            width: weatherLoader.implicitWidth + 16
                ...
                gradient: Gradient {
                 ...
                 Loader {
                    ...
                    Binding {
                        ...
                        when: weatherLoader.status === Loader.Ready && root.barWindow !== undefined
                        restoreMode: Binding.RestoreBinding
                    }
                    ...
                    Binding {
                        ...
                        when: weatherLoader.status === Loader.Ready && root.weatherPopup !== undefined
                        restoreMode: Binding.RestoreBinding
                    }
            ...
            Rectangle {
             ...
             Loader {
                ...
                Binding {
                    ...
                    property: "barWindow"
                    value: root.barWindow
                    when: clockLoader.status === Loader.Ready && root.barWindow !== undefined
                    restoreMode: Binding.RestoreBinding
                }
                ...
                Binding {
                    ...
                    value: root.calendarPopup
                    when: clockLoader.status === Loader.Ready && root.calendarPopup !== undefined
                    restoreMode: Binding.RestoreBinding
                }
        ...
        Row {
            ...
            Rectangle {
             ...
             Rectangle {
                 ...
                 Loader {
                     id: settingsCogLoader
                     anchors.centerIn: parent
                     asynchronous: true
                     source: "components/SettingsCog.qml"

                     Binding {
                         target: settingsCogLoader.item
                         property: "settingsPopup"
                         value: root.settingsPopup
                         when: settingsCogLoader.status === Loader.Ready && root.settingsPopup !== undefined
                         restoreMode: Binding.RestoreBinding
                     }
                     ...
                     Binding {
                        ...
                        property: "barRoot"
                        value: root
                        when: settingsCogLoader.status === Loader.Ready
                    }
                    ...
                    Binding {
                        ...
                        property: "barWindow"
                        value: root.barWindow
                        when: settingsCogLoader.status === Loader.Ready && root.barWindow !== undefined
                    }
                 }
             }
        }
